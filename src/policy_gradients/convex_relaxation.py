...
if forward_one:
    def get_kl_bound(model, x, means, eps, beta=None, stdev=None, use_full_backward=False):
        ...
        x = BoundedTensor(x, ptb=PerturbationLpNorm(norm=np.inf, eps=eps)).to(boundedmodule_device).requires_grad_(False)
        if forward_one:
            inputs = (x, )
        else:
            inputs = (x, means.to(boundedmodule_device))
    ...
    def get_kl_bound(model, x, means, eps):
        # Set each layer's perturbation eps and log_stdev's perturbation.
        x = BoundedTensor(x, ptb=PerturbationLpNorm(norm=np.inf, eps=eps)).to(boundedmodule_device)
        if forward_one:
            inputs = (x, )
        else:
            inputs = (x, means.to(boundedmodule_device))
...
def compute_perturbations(model, x, means, perturbations):
    use_ibp = True
    method = 'backward'
    x = BoundedTensor(x, ptb=PerturbationLpNorm(norm=np.inf, eps=0)).to(boundedmodule_device)
    inputs = (x, means.to(boundedmodule_device))
