...
if forward_one:
    def get_kl_bound(model, x, means, eps, beta=None, stdev=None, use_full_backward=False):
        ...
        x = BoundedTensor(x, ptb=PerturbationLpNorm(norm=np.inf, eps=eps)).requires_grad_(False).to(x.device)
        if forward_one:
            inputs = (x, )
        else:
            inputs = (x, means)
    ...
    def get_kl_bound(model, x, means, eps):
        ...
        x = BoundedTensor(x, ptb=PerturbationLpNorm(norm=np.inf, eps=eps)).to(x.device)
        if forward_one:
            inputs = (x, )
        else:
            inputs = (x, means)
...
def compute_perturbations(model, x, means, perturbations):
    ...
    x = BoundedTensor(x, ptb=PerturbationLpNorm(norm=np.inf, eps=0)).to(x.device)
    inputs = (x, )
