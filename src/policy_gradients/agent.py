...
class Trainer():
    ...
    def create_relaxed_model(self, time_in_state=False):
        ...
        if isinstance(self.policy_model, CtsPolicy):
            if self.ROBUST_PPO_METHOD == "convex-relax":
                ...
                inputs = (dummy_input1, )
                self.relaxed_policy_model = BoundedModule(relaxed_policy_model, inputs, device = boundedmodule_device)
            else:
    ...
    def setup_sarsa(self, lr_schedule, eps_scheduler, beta_scheduler):
        ...
        self.relaxed_sarsa_model = BoundedModule(self.sarsa_model, dummy_input, device = boundedmodule_device)
    ...
    def multi_actor_step(self, actions, envs):
        ...
        for action, env in zip(actions, envs):
            ...
            # new_state, normed_reward, terminated, truncated, info = env.step(gym_action)
