...
def calculate_total_discounted_return(episode_step_rewards, gamma):
    step_rews = np.array(episode_step_rewards)
    return np.sum(gamma**np.arange(len(step_rews)) * step_rews)
...
class Trainer():
    ...
    def __init__(self, policy_net_class, value_net_class, params,
        ...
        if self.params.ATTACK_EPS == 'nuus':
            print(f'NUUS Solver: {self.params.NUUS_SOLVER}')
            if self.params.NUUS_SOLVER == 'cvxpy': 
                print('Creating NUUS_GPSolverNumpy!')
                self.nuus_solver = NUUS_GPSolverNumpy(self.params.NUM_FEATURES, self.ROBUST_PPO_EPS)
    ...
    def create_relaxed_model(self, time_in_state=False):
        ...
        if isinstance(self.policy_model, CtsPolicy):
            if self.ROBUST_PPO_METHOD == "convex-relax":
                ...
                self.relaxed_policy_model = BoundedModule(relaxed_policy_model, inputs, device = 'cpu' if self.CPU else 'cuda')
            else:
    ...
    def setup_sarsa(self, lr_schedule, eps_scheduler, beta_scheduler):
        ...
        self.relaxed_sarsa_model = BoundedModule(self.sarsa_model, dummy_input, device = 'cpu' if self.CPU else 'cuda')
    ...
    def run_trajectories(self, num_saps, return_rewards=False, should_tqdm=False,
        ...
        for t in iterator:
            ...
            if is_advpolicy_training:
                ...
                maybe_attacked_last_states, _ = self.apply_attack(last_states)
        ...
        if collect_perturbed_state:
            if is_advpolicy_training:
                ...
                last_states, _ = self.apply_attack(last_states)
    ...
    def apply_attack(self, last_states):
        ...
        if self.params.ATTACK_METHOD == "critic":
            ...
            if steps > 0:
                ...
                with torch.enable_grad():
                    for i in range(steps):
                        ...
                        states.data = torch.min(torch.max(states.data - update, clamp_min), clamp_max)
                    self.val_model.zero_grad()
                return states.detach(), eps
            else:
                return last_states, eps
        elif self.params.ATTACK_METHOD == "random":
            # Apply an uniform random noise.
            # noise = torch.empty_like(last_states).uniform_(-eps, eps)
            noise = torch.empty_like(last_states).uniform_(-1, 1) * eps
            return (last_states + noise).detach(), eps
        elif self.params.ATTACK_METHOD == "action" or self.params.ATTACK_METHOD == "action+imit":
            if steps > 0:
                if self.params.ATTACK_STEP_EPS == "auto":
                    step_eps = eps / steps
            ...
            perturbed_states = last_states + ch.tanh(next_adv_perturbations) * eps
            """
            return perturbed_states.detach(), eps
        elif self.params.ATTACK_METHOD == "none":
            return last_states, eps
        else:
            raise ValueError(f'Unknown attack method {self.params.ATTACK_METHOD}')
    ...
    def run_test_trajectories(self, max_len, should_tqdm=False, deterministic=False):
        ...
        for t in iterator:
            ...
            maybe_attacked_last_states, eps = self.apply_attack(last_states)
            ...
            if self.params.ATTACK_METHOD != "none":
                ...
                if torch.any(state_diff > eps + 1e-5):
