...
from .steps import value_step, step_with_mode, pack_history, advantage_step
from .logging import *
...
class Trainer():
    ...
    def sarsa_steps(self, saps):
        ...
        for i in range(self.params.VAL_EPOCHS):
            ...
            for selected in splits:
                ...
                bounded_inputs = BoundedTensor(inputs, ptb=PerturbationLpNorm(norm=np.inf, eps=self.sarsa_eps_scheduler.get_eps())).to('cpu' if self.CPU else 'cuda')
    ...
    def run_test(self, max_len=2048, compute_bounds=False, use_full_backward=False, original_stdev=None):
        ...
        with torch.no_grad():
            ...
            # Unroll the trajectories (actors, T, ...) -> (actors*T, ...)
        return ep_length, ep_reward, ep_return, ep_ori_return, actions.cpu().numpy(), action_means.cpu().numpy(), states.cpu().numpy(), kl_upper_bound
