...
def condor_train_tasks(root_dir = 'run_exp_inputs/'):
    ...
    for seed in [1, 23, 42, 233, 234, 236, 667, 888][:4]:
        for env in ['hopper', 'halfcheetah', 'walker', 'humanoid'][:-1]:
            for algo in ['radial_ppo'][:]:
                content = []
...
# Testing tasks
def condor_test_tasks(exp_root_dir = 'exps/', algo = 'radial_ppo', test_task_root_dir = 'test_exp_inputs/'):
    eps = {
        'Walker2d-v5' : 0.05,
